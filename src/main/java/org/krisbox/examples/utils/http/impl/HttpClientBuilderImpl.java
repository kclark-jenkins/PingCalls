...
public class HttpClientBuilderImpl extends PingPropertiesImpl implements HttpClientBuilder {
    public HttpClientBuilderImpl() throws IOException { debug("HttpClientBuilderImpl()");}
    ...
    public Object buildHttpClient(HashMap<HttpClientFields, HashMap<HttpClientFields, Object>> requestData) {
        ...
        if(requestData.get(HttpClientFields.Client).get(HttpClientFields.Type).equals("GET")) {
            HttpGet httpRequest = new HttpGet(getPingGet() + requestData.get(HttpClientFields.Client).get(HttpClientFields.Endpoint));
    ...
    private Header createAuthHeader() { debug("HttpClientBuilderImpl.createAuthHeader()");return new BasicHeader("Authorization", "Basic " + DatatypeConverter.printBase64Binary(new String(getClientID() + ":" + getApiKey()).getBytes())); }
